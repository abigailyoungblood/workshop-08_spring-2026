...
drought_exp_clean <- drought_exp |> 
  # making names code-readable in new object
  clean_names() |> 
  # 
  ...
  # creating new column with full treatment
...
ggplot(data = drought_exp_clean,
  ...
  geom_jitter(height = 0,
              width = 0.1) +
  labs(title = "Well watered individuaks tend to have higher mass than drought stressed",
       x = "Treatment",
       y = "total plant mass (g)") +
...
model1 <- lm(total_g ~ sla + water_treatment,
             data = drought_exp_clean)

# model 3: SLA and species
model1 <- lm(total_g ~ sla + species_name,
             data = drought_exp_clean)

# model 4: water treatment and species
model1 <- lm(total_g ~ water_treatment + species_name,
             data = drought_exp_clean)
...
model1 <- lm(total_g ~ water_treatment,
             data = drought_exp_clean)

# model 7: species
model1 <- lm(total_g ~ species_name,
             data = drought_exp_clean)
